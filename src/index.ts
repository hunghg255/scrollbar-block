 ...
 */
function scrollbarBlock(node = null) {
  const mouseOver = () => {
  ...
  if (lockedScrolls.length > 0) {
    ...
    lockedScrolls = lockedScrolls.filter((lockedScroll) => document.body.contains(lockedScroll.node));
  }
  ...
  return () => {
    ...
    if (index > -1) {
      ...
      lockedScrolls = lockedScrolls.filter((lockedScroll) => {
        const containsLockedScrollNode = document.body.contains(lockedScroll.node);
